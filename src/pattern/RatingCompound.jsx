...
const Comment = () => {
  ...
  return (
    <div className="w-full mt-2 ">
      <label className="p-2 text-base text-black font-semibold ">Comment</label>
      <form className="relative " onSubmit={handleSubmit}>
        <textarea
          ...
          className="w-full text-slate-500 p-4 rounded-md resize-none min-h-20"></textarea>
  ...
  return (
    <div className="absolute top-0 px-8 py-2 translate-x-full bg-white rounded-md max-w-[300px] -right-5">
      {userFeedback.length > 0 ? (
        <>
          <h3 className="mb-2 text-black text-xl font-semibold">
            Here are what user think
          </h3>
          <ul>
            {userFeedback.map((user, index) => (
              <li key={index} className="px-2 text-slate-500">
                <h4>
                  {index + 1}.{" "}
        ...
        <p className="font-semibold ext-xl text-black">No user feedback yet...</p>
      )}
    </div>
  );
};
MultiRatings.UserFeedback = UserFeedback;

MultiRatings.Label = Label;
MultiRatings.RatingsContainer = RatingsContainer;
MultiRatings.RatingLabel = RatingLabel;
MultiRatings.Comment = Comment;

export default MultiRatings